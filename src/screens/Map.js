...
class CrimeDescription extends Component {
  ...
  render(){
      ...
      <ScrollView>
          <CheckBoxCustom title={"Personal Crime"} examples={"Assault, Battery, Homicide, etc"} onPress={() => this.setState({Personal_Crime : !this.state.Personal_Crime})} checked={this.state.Personal_Crime}/>
          <CheckBoxCustom title={"Property Crime"} examples={"Theft, Arson, Forgery, etc"} onPress={() => this.setState({Property_Crime : !this.state.Property_Crime})} checked={this.state.Property_Crime}/>
          <CheckBoxCustom title={"Victimless Crime"} examples={"Gambling, drug use, etc"} onPress={() => this.setState({Victimless_Crime : !this.state.Victimless_Crime})} checked={this.state.Victimless_Crime}/>
          <CheckBoxCustom title={"Organised Crime"} examples={"Terrorism, etc"} onPress={() => this.setState({Organised_Crime : !this.state.Organised_Crime})} checked={this.state.Organised_Crime}/>
          <CheckBoxCustom title={"White-Collar Crime"} examples={"Insider trading, Tax evasion, etc"} onPress={() => this.setState({White_Collar_Crime : !this.state.White_Collar_Crime})} checked={this.state.White_Collar_Crime}/>
          <CheckBoxCustom title={"Hate Crime"} examples={"Crimes motivated by bias against race, caste, religion, national origin, etc"} onPress={() => this.setState({Hate_Crime : !this.state.Hate_Crime})} checked={this.state.Hate_Crime}/>

          <TextInput style={{width:"90%",backgroundColor:"#8899dd33",paddingRight:15,paddingLeft:15,marginTop:"10%",
          alignSelf:"center",alignContent:"center",borderColor:"#ffffff33",borderWidth:1,borderRadius:20,
          color:"#ffffff99",textAlignVertical: 'top'}}
              multiline={true}
              placeholder="Details"
          ...
          <View>
          <TextInput 
              style={{left:"20%",marginTop:"8%",width:"60%",backgroundColor:"#8899dd33",paddingRight:15,paddingLeft:15,
              textAlignVertical: 'top',borderColor:"#ffffff33",borderWidth:1,borderRadius:20,
              color:"#ffffff99",left:"5%"}}
              multiline={true}
          ...
          <NextButton style={{bottom:'30%',width:"10%",marginBottom:20,left:"80%",}} onPress={() => {
              crime = '';
              ...
              if(this.state.Crime_Description != '') {
                  ...
                  Alert.alert('Provide Info !','Please Give Crime Description');
                  return;
              }
              crimeSuspect = '';
...
reverseGeo(coordinate){
  ...
  render() {
    ...
    if (this.state.locationChosen) {
      marker = <MapView.Marker coordinate={this.state.focusedLocation} title={this.state.locationAdd}/>;
...
class PhotoForPothole extends Component {
  ...
  state = {
    ...
    imgName: '',
    loading:false,
    videoUri:'http://anthillonline.com/wp-content/uploads/2013/07/videoPlaceholder.jpg'
}
  ...
  getImage() {
    ...
    uploadImage = (uri, mime = 'application/octet-stream') => {
      this.setState({loading : true});
        ...
        const imageRef = Firebase.storage().ref('/crimeImages/').child(this.state.imgName);
  ...
  render() {
    return (
      <View style={styles.imgUploadContainer}>
          ...
          <View style={{borderRadius:10,marginTop:10,alignSelf:'center',width:"95%",backgroundColor:"#2a475e",height:"45%"}}>
        ...
        colors={[ '#04C204','#089608']} style={styles.gradient}>
              ...
              <Image source={{uri: this.state.imageUri}} style={{marginTop:5,height: '60%', width: '95%',alignContent:'center',alignSelf:'center',alignItems:'center',top:"5%"}}/>
          </View>
          {this.loadSpinner()}
          <View style={{borderRadius:10,marginTop:10,alignSelf:'center',width:"95%",backgroundColor:"#2a475e",height:"45%"}}>
          <TouchableOpacity style={styles.welcome} onPress={this.getImage.bind(this)}>
          <LinearGradient   start={{x: 0.25, y: 0}} end={{x: 1, y: 0.5}}
            ...
            <Text style={{color:'white'}}>Click to upload video</Text>
              ...
              <Image source={{uri: this.state.videoUri}} style={{marginTop:5,height: '60%', width: '95%',alignContent:'center',alignSelf:'center',alignItems:'center',top:"5%"}}/>
              {this.loadSpinner()}
          </View>
      </View>
  );
...
class Done extends Component {
  ...
  uploadReport = () => {
    ...
    if(reportStatus == true) {
      ...
      .push({
        ...
        crimeType : crime,
        crimeDescription,crimeSuspect,
        crimeImage : getDownloadImgURL,
        reportedAt : Date().toLocaleString() 
      })
      .then(() => {
          Firebase.database().ref(`crimeReports/`)
          .push({
  ...
  render() {
    return (
      <View style={{flex:1}}>
        <Text style={{alignSelf:'center',top:'40%',fontSize:25,left:'5%',color:'red'}}>
          * False report may 
          contain account suspension.
        </Text>
        {this.loadSpinner()}
        <TouchableOpacity style={styles.doneuploading} onPress={this.uploadReport}>
